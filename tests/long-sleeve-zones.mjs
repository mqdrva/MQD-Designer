...
// Independent measured forearm samples: both sides must stay sleeve-only.
for(const sign of [-1,1])for(const [x,y] of [[.456,-.1],[.477,-.2],[.507,-.3],[.54,-.4],[.625,-.6]]){const tri=[[sign*x,y,.02],[sign*(x+.001),y,.02],[sign*x,y+.001,.021]];assert(partition(tri).every(([zone])=>zone===(sign===1?2:3)));}
