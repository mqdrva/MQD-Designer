...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const ids=[...new Set(placements.map(row=>row.asset_id))],{data:assets,error:assetError}=await supabase.from('mqd_library_assets').select('id,name,slug,category,placement_mode,placement_preset,stack_order,render_path,sort_order').in('id',ids).eq('active',true).order('stack_order').order('sort_order').order('name');
