...
old_decl = "let scene,camera,renderer,controls,garment=null,decalGroup=null,editorZoom=1,showGrid=false,dragState=null;"
new_decl = "let scene,camera,renderer,controls,garment=null,decalGroup=null,editorZoom=1,showGrid=true,dragState=null;\nconst templateCache=new Map();"
if old_decl in s:
    s = s.replace(old_decl, new_decl, 1)

if 'function ensureTemplateImage(' not in s:
    insert_after = "function repairImageObjects(){Object.values(designs).forEach(ps=>Object.values(ps.zones||{}).forEach(z=>z.layers?.forEach(l=>{if(l.type==='image'&&l.src&&!l.image){const img=new Image();img.onload=renderAll;img.src=l.src;l.image=img;}})));}"
    helpers = r'''
...
'''
    if insert_after not in s:
        raise SystemExit('Could not insert v21 helpers')
    s = s.replace(insert_after, insert_after + '\n' + helpers, 1)

s = replace_function(s, 'drawZoneComposite', r'''
...
s = replace_function(s, 'makeZoneTextureCanvas', r'''
...
s = replace_function(s, 'zonePlacement', r'''
...
s = replace_function(s, 'selectProduct', r'''
...
s = replace_function(s, 'selectZone', r'''
...
old_export = "drawZoneComposite(x,w,h,false);"
if old_export not in s:
    raise SystemExit('Production export call not found')
s = s.replace(old_export, "drawProductionZone(x,z,w,h);", 1)

if "clearDecals();new GLTFLoader().load(product.model" in s:
    s = s.replace("clearDecals();new GLTFLoader().load(product.model", "clearDecals();preloadTemplates();new GLTFLoader().load(product.model", 1)
